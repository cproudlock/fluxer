...
| Property | Type | Description |
|----------|------|-------------|
| consistency? | enum&lt;`local_one`, `local_quorum`, `quorum`, `one`, `all`&gt; | Default read/write consistency level. Default: `local_one` |
...
| fcm? | [fcm_integration](#fcm-integration) | Default: `{}` |
| gif? | [gif](#gif) | Default: `{}` |
...
| native_bypass_secret? | string | HMAC secret for native app captcha bypass. Must be at least 32 characters. Native apps sign a server-issued nonce with this secret to prove they are legitimate. Default: `""` |
...
| reconciliation_enabled? | boolean | Enable the voice reconciliation worker that cleans up orphaned LiveKit participants. Disable on standby nodes that share LiveKit servers but have separate Valkey instances. Default: `true` |
...
JSON path: `integrations.fcm`

FCM push notification integration for Android devices.

| Property | Type | Description |
|----------|------|-------------|
| enabled? | boolean | Enable FCM push notifications for Android devices. Default: `false` |
| service_account_key_path? | string | Path to Google service account JSON key file for FCM. |
...
| voice_kv? | string | Shared Valkey/Redis URL for voice connection state. When set, voice state is stored here instead of the local kv, enabling cross-node reconciliation. |
...
| enabled? | boolean | Whether to start the embedded gateway process. Set to false when using a dedicated gateway container. Default: `true` |
