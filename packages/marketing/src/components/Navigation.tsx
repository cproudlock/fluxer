	...
	return (
		<nav id="navbar" class="fixed top-0 right-0 left-0 z-40">
			...
			<div class="px-6 py-4 sm:px-8 md:px-12 md:py-5 lg:px-8 xl:px-16">
				<div class="mx-auto max-w-7xl rounded-2xl border border-[#1E293B]/60 bg-[#101931]/95 px-3 py-2 shadow-lg backdrop-blur-lg md:px-5 md:py-2.5">
					<div class="flex items-center justify-between">
						<div class="flex items-center gap-4 xl:gap-6">
							...
							>
								<FluxerLogoWordmarkIcon class="h-8 text-[#3B82F6] md:h-9" />
								<span class="absolute right-0 -bottom-1.5 whitespace-nowrap rounded-full border border-white bg-[#3B82F6] px-1.5 py-0.5 font-bold text-[8px] text-white leading-none">
							...
							<div class="marketing-nav-links hidden items-center gap-4 lg:flex xl:gap-6">
								<a
									...
									class="body-lg font-semibold text-white/90 transition-colors hover:text-white"
								>
									{ctx.i18n.getMessage('download.download', ctx.locale)}
								</a>
								<a
									href={href(ctx, '/reverb')}
									class="body-lg font-semibold text-white/90 transition-colors hover:text-white"
								>
						...
						<div class="flex items-center gap-1 xl:gap-2">
							<a
								...
								class="hidden items-center rounded-lg p-2 text-[#3B82F6] transition-colors hover:bg-[#1A2440] hover:text-[#2563EB] lg:flex"
								target="_blank"
							...
							<a
								href="https://github.com/cproudlock/fluxer"
								class="hidden items-center rounded-lg p-2 text-[#3B82F6] transition-colors hover:bg-[#1A2440] hover:text-[#2563EB] lg:flex"
								target="_blank"
								rel="noopener noreferrer"
							...
							<LocaleSelectorTrigger
								ctx={ctx}
								className="hidden text-[#3B82F6] transition-colors hover:bg-[#1A2440] hover:text-[#2563EB] lg:flex"
							/>
							<MarketingButton
							...
							<label
								for="nav-toggle"
								class="relative z-10 flex cursor-pointer items-center justify-center rounded-lg p-2 transition-colors hover:bg-[#1A2440] lg:hidden"
							>
								<MenuIcon class="h-6 w-6 text-white peer-checked:hidden" />
							</label>
						</div>
					</div>
				</div>
			</div>
			<div class="pointer-events-none fixed inset-0 z-50 bg-[#0A1428]/50 opacity-0 backdrop-blur-sm transition-opacity peer-checked:pointer-events-auto peer-checked:opacity-100 lg:hidden">
				<button
					type="button"
			...
			<div class="fixed top-0 right-0 bottom-0 z-50 w-full translate-x-full transform overflow-y-auto rounded-none bg-[#101931] shadow-2xl transition-transform peer-checked:translate-x-0 sm:w-[420px] sm:max-w-[90vw] sm:rounded-l-3xl lg:hidden">
				<div class="flex h-full flex-col p-6">
					<div class="mb-6 flex items-center justify-between">
						<a
							href={href(ctx, '/')}
							class="flex items-center gap-3 rounded-xl px-2 py-1 transition-colors hover:bg-[#1A2440]"
							aria-label={ctx.i18n.getMessage('navigation.go_home', ctx.locale)}
						>
							<FluxerLogoWordmarkIcon class="h-7 text-[#3B82F6]" />
						</a>
						<label for="nav-toggle" class="cursor-pointer rounded-lg p-2 transition-colors hover:bg-[#1A2440]">
					...
					<div class="-mx-2 flex-1 overflow-y-auto px-2">
						<div class="flex flex-col gap-6">
							<div>
								<p class="mb-2 font-semibold text-[#94A3B8] text-xs uppercase tracking-wide">
									{ctx.i18n.getMessage('company_and_resources.product', ctx.locale)}
								</p>
								<div class="flex flex-col gap-1">
									<a
										href={href(ctx, '/download')}
										class="rounded-lg py-2.5 pr-3 pl-0 font-semibold text-base text-white transition-colors hover:bg-[#1A2440]"
									>
										{ctx.i18n.getMessage('download.download', ctx.locale)}
									</a>
									<a
						...
						<div class="mt-4 flex flex-col gap-3 border-[#1E293B] border-t pt-4">
							<MobileDrawerButton
...
function MobileDrawerButton(props: MobileDrawerButtonProps): JSX.Element {
	...
	const baseClass =
		'flex w-full items-center justify-start gap-3 rounded-lg px-3 py-2.5 font-semibold text-base text-white transition-colors hover:bg-[#1A2440] lg:hidden';
